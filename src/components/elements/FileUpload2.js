...
import getValueInSchema from '../utils/getValueInSchema';
...
const FileUpload = ({ adamant_field_error, adamant_error_description, contentEncoding, withinObject, field_uri, dataInputItems, setDataInputItems, withinArray, path, pathFormData, field_required, field_index, edit, field_key, field_label, field_description, field_enumerate, defaultValue, value }) => {
    ...
    const { handleRemoveFile, loadedFiles, handleLoadedFiles, updateParent, convertedSchema, handleDataDelete, handleConvertedDataInput } = useContext(FormContext);
    ...
    useEffect(() => {
        ...
        if (typeof value === 'string') {
            let arr = value.split(";")
            let isFileUpload = value.includes("fileupload:")
            if (arr.length === 3 && isFileUpload) {
    ...
    const handleDeleteElement = () => {
        ...
        const fileIndex = getFileIndex(loadedFiles, mediaFileType)
        ...
        (acceptedFile) => {
            ...
            if (valid) {
                ...
                for (let i = 0; i < loadedFiles.length; i++) {
                    if (loadedFiles[i]["name"] === acceptedFile[0]["name"] && loadedFiles[i]["size"] === acceptedFile[0]["size"]) {
                ...
                if (isFileAlreadyExist) {
                    ...
                    setInputError(true)
                    alert("You have loaded this file! Loaded files must be unique.")
                }
                else {
                    setFileSize(acceptedFile[0]["size"])
                    setFileName(acceptedFile[0]["name"])
                    // no need to read the file now, just collect everything first in loaded files state variable
                    if (withinArray !== undefined & withinArray) {
                        // remove the file in loadedFiles
                        if (value !== undefined && value !== "") {
                            let arrayedPath = path.split(".")
                            let lastElementPath = arrayedPath.pop()
                            arrayedPath.push("value")
                            arrayedPath.push(lastElementPath)
                            arrayedPath.push(field_key)
                            const fileMetadata = getValueInSchemaFullPath(convertedSchema, arrayedPath.join("."))
                        ...
                        handleLoadedFiles(acceptedFile[0])
                        ...
                        items[field_index][field_key] = "fileupload:" + acceptedFile[0]["type"] + ";" + acceptedFile[0]["name"] + ";" + acceptedFile[0]["size"];
                        ...
                        // remove the file in loadedFiles
                        const fileMetadata = getValueInSchemaFullPath(convertedSchema, path + `.value`)
                        if (fileMetadata !== undefined) {
                            const fileIndex = getFileIndex(loadedFiles, fileMetadata)
                            handleRemoveFile(fileIndex)
                        }
                        ...
                        handleLoadedFiles(acceptedFile[0])
                        handleConvertedDataInput("fileupload:" + acceptedFile[0]["type"] + ";" + acceptedFile[0]["name"] + ";" + acceptedFile[0]["size"], path + ".value", "boolean")
                ...
                <div style={{ paddingLeft: "15px", width: "100%" }}>
                    <FormControl >
                        ...
                        <div style={{ width: "225px", display: "flex", justifyContent: "center" }}>
                            {/*mediaFileType !== "" ? <img alt='fileUpload' src={FileIcon} width={"64px"} /> : ""*/}
                            {mediaFileType !== "" ? <div style={{ width: "30%" }}><FileIcon extension={fileName.split(".").slice(-1)[0]} {...defaultStyles[fileName.split(".").slice(-1)[0]]} /> </div> : ""}
                        </div>
                        <div style={{ width: "225px", fontSize: "10px", color: "grey", paddingTop: "5px", whiteSpace: "nowrap", textOverflow: "ellipsis", overflow: "hidden", textAlign: "center" }}>
                            {fileName !== undefined ? fileName : null}
                        </div>
                        <div style={{ width: "225px", fontSize: "10px", color: "grey", paddingTop: "5px", whiteSpace: "nowrap", textOverflow: "ellipsis", overflow: "hidden", textAlign: "center" }}>
                            {fileSize !== undefined ? `${fileSize / 1000} KB` : null}
                        </div>
                        <div style={{ paddingTop: "5px", textAlign: "left", width: "100%" }} >
                            ...
                            {fileName !== "" ? <Button onClick={() => { handleOnClickedClear() }} style={{ marginLeft: "5px" }} variant="outlined" color="secondary">Clear</Button> : null}
                        </div>
